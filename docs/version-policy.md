...
| Prometheus | `prom/prometheus` | `v3.5.4` | Stable Prometheus 3 release branch. |
...
| Python base image | `python` | `3.12-slim` | Lightweight runtime for the MSA demo services in `msa-demo`. |

## Upgrade Rule

Upgrade one component family at a time:

1. Update `.env.monitoring.example`, `.env.app.example`, and fallback tags in `docker-compose.monitoring.yml` or `docker-compose.app.yml`.
2. Run `docker compose --env-file .env.monitoring.example config` on a workstation.
3. Run `docker compose --env-file .env.app.example config` on a workstation.
4. Start the affected VM stack in a clean test environment.
5. Run `bash scripts/healthcheck.sh`.
6. Generate MSA traffic with `bash scripts/random-demo-traffic.sh`.
7. Check Grafana datasource health, dashboards, logs, metrics, and traces.
