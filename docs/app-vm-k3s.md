...
Monitoring VM
  ...
  - Prometheus, Loki, Tempo, Mimir, Grafana, Alertmanager
...
Prometheus scrape 대상 포트는 Monitoring VM의 기존 scrape 설정과 동일하게 유지한다.

```text
api-service       app-vm:8080
catalog-service   app-vm:8081
inventory-service app-vm:8082
cart-service      app-vm:8083
order-service     app-vm:8084
payment-service   app-vm:8085
node-exporter     app-vm:9100
```
...
Monitoring VM에서 Prometheus scrape 경로를 확인한다.
...
- `Alerts Overview`: 서비스 down, node-exporter down, error rate alert 확인
...
Node Exporter와 Promtail 시나리오는 DaemonSet 삭제/재적용으로 테스트한다.
